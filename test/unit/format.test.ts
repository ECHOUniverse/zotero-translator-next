...
describe("formatText 格式化管线", () => {
  ...
  describe("修复连字符断词 (fixHyphenation)", () => {
    it("inter-\\nnational → international", () => {
      expect(
        formatText("inter-\nnational", { fixHyphenation: true }),
      ).to.equal("international");
    });
    ...
    it("数字范围连字符不动", () => {
      expect(
        formatText("pages 10-\n20", { fixHyphenation: true }),
      ).to.equal("pages 10-20");
    });
  });
  ...
  describe("引号正常化 (normalizeQuotes)", () => {
    ...
    it("英文单引号 → 中文单引号", () => {
      expect(
        formatText("'quoted' text", { normalizeQuotes: true }),
    ...
    it("«» 转为中文引号", () => {
      expect(
        formatText("«bonjour»", { normalizeQuotes: true }),
      ).to.equal("“bonjour”");
    });
    it("孤立的闭引号修复为开引号", () => {
      expect(
        formatText("he said”hello", { normalizeQuotes: true }),
      ).to.equal("he said“hello");
    });
  });
  ...
  describe("破折号正常化 (normalizeDashes)", () => {
    it("-- → —", () => {
      expect(
        formatText("a -- b", { normalizeDashes: true }),
      ).to.equal("a — b");
    });
    it("连写 word--word → word—word", () => {
      expect(
        formatText("word--word", { normalizeDashes: true }),
      ).to.equal("word—word");
    });
    it("en dash – → —", () => {
      expect(
    ...
    it("单连字符保留", () => {
      expect(
        formatText("well-known", { normalizeDashes: true }),
      ).to.equal("well-known");
    });
  });
  ...
  describe("全半角统一 (normalizeWidth)", () => {
    it("全角字母数字 → 半角", () => {
      expect(
        formatText("ＡＢＣ１２３", { normalizeWidth: true }),
      ).to.equal("ABC123");
    });
    it("全角空格 → 半角空格", () => {
      expect(
        formatText("a\u3000b", { normalizeWidth: true }),
      ).to.equal("a b");
    });
    it("中文标点不受影响", () => {
      expect(
        formatText("你好，世界！", { normalizeWidth: true }),
      ).to.equal("你好，世界！");
    });
  });
  ...
  describe("空白压缩 (collapseWhitespace)", () => {
    it("多空格 → 单空格", () => {
      expect(
        formatText("a   b    c", { collapseWhitespace: true }),
      ).to.equal("a b c");
    });
    it("多空行 → 单空行", () => {
      expect(
        formatText("a\n\n\n\nb", { collapseWhitespace: true }),
      ).to.equal("a\n\nb");
    });
    it("去除零宽字符与 BOM", () => {
      expect(
    ...
    it("首尾去空白", () => {
      expect(
        formatText("  padded  ", { collapseWhitespace: true }),
      ).to.equal("padded");
    });
  });
  ...
  describe("特殊符号正常化 (normalizeSymbols)", () => {
    it("数学减号 − → -", () => {
      expect(
        formatText("a − b", { normalizeSymbols: true }),
      ).to.equal("a - b");
    });
    it("全角连字符 － → -", () => {
      expect(
