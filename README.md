...
- **多渠道翻译 + 自动回退**：MyMemory（免费免 key）→ 有道 / 腾讯交互翻译 / 火山网页 → Bing（Edge 免费）→ Google → DeepSeek → 腾讯云 TMT → 百度 / 阿里云机翻 / Qwen-MT → 自定义 OpenAI 兼容渠道；失败按配置顺序自动回退，429 限流指数退避
- **流式体验**：AI 渠道逐字流式输出；翻译队列 + 取消；划选自动翻译（默认关，可调防抖）；LLM 渠道上下文感知
- **翻译历史**：持久化到 Zotero 数据库；**侧栏只显示当前文章历史**（分区标题与当前翻译明确区隔，匹配当前任务的记录标「当前」徽标）+ 单条删除/清空本条；「查看全部历史」独立窗口按文章 tab 浏览全部记录；相同文本自动命中缓存（可关）
...
| Youdao     | 无需配置               | 有道 demo 网页端点，国内可达；未文档化，失效走回退                                                                         |
| Transmart  | 无需配置               | 腾讯交互翻译网页端点，中英等；未文档化，失效走回退                                                                         |
| Volcengine | 无需配置               | 火山引擎网页 crx 端点；未文档化，失效走回退                                                                                |
| Bing       | 无需配置（Edge 模式）  | Edge 内置翻译接口，免费匿名；可选 Azure Key 官方模式                                                                       |
| Google     | 无需配置               | 非官方 gtx 端点，语种最全；大陆常不可达，失败走回退                                                                        |
...
| 百度翻译   | App ID + 密钥          | 标准版约 5 万字符/月；个人认证高级版约 100 万字符/月                                                                       |
| 阿里云机翻 | AccessKey              | 通用版约 100 万字符/月免费（[计费说明](https://help.aliyun.com/zh/machine-translation/product-overview/billing-overview)） |
| Qwen-MT    | DashScope API Key      | 百炼机器翻译模型；新人约 90 天免费额度（仅翻译，不可用于 AI 总结）                                                         |
...
- 翻译文本会发送至所选第三方服务（MyMemory / 有道 / Transmart / 火山 / Bing / Google / DeepSeek / 腾讯云 TMT / 百度 / 阿里云 / Qwen-MT / 自定义）
